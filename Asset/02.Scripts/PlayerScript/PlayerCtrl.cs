   ...
   public float dashCoolTime = 2.0f; // 대쉬 사용가능 쿨타임
    ...
    void Update() //각 스크립트 활성 비활성으로 구현, update 순서 가장 먼저하게 고정해서 state를 고정함으로써 입력 움직임간 지연 없애기. 오류 방지. 점프 벽점프 대쉬는 playerctrl에 잇음.
    {
        if (state != State.DIE)
        {
            ...
            if (state == State.WALLRUN) //벽타기 도중
            {
                if (!(isWall))
                {
                    ChangeState(PlayerCtrl.State.IDLE);
    ...
    void SpaceInput()
    {
        ...
        if (Input.GetKeyUp(KeyCode.Space) || Input.GetButtonUp("Jump"))
            isSpaceUp = true;
        else if (Input.GetKeyDown(KeyCode.Space) || Input.GetButtonDown("Jump"))
        {
            isSpaceDown = true;
        }
    ...
    public void Die()
    {
        ...
        gameObject.GetComponent<PlayerRewind>().Load();
        //플레이어 스테이터스 초기화
    }
    ...
    void PlayerFall()
    {
        ...
        if (tr.position.y < 180 && scene.name != "DebugScene" && scene.name != "XR_DebugScene" && scene.name != "VR Tutorial level")
        {
            Die();
        }
