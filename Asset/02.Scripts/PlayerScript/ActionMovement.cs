...
public class ActionMovement : MonoBehaviour
{
    ...
    void Update()
    {
        if(ps.state == PlayerCtrl.State.IDLE)
        {
            if(ps.isSpaceDown)
            {
                if (ps.jumpAvailable)
        ...
        if(ps.state != PlayerCtrl.State.DIE && ps.state != PlayerCtrl.State.RUSH)
        {
    ...
    void Dash()
    {
        ...
        {
            Vector3 dir = Camera.main.transform.right * h + Camera.main.transform.forward * v + Camera.main.transform.up * 0.1f;
            if (h == 0 && v == 0)
            {
